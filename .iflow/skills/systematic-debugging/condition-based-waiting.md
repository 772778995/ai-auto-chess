...
不稳定的测试经常猜测时机使用任意延迟。这创造了竞态条件，测试在快速机器上通过但在负载下或 CI 中失败。
...
参见本目录中的 `condition-based-waiting-example.ts` 了解完整实现，包含来自实际调试会话的领域特定助手（`waitForEvent`、`waitForEventCount`、`waitForEventMatch`）。
...
// 200ms = 100ms 间隔的 2 次滴答 - 已文档化并有理由
```
