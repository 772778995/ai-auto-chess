...
description: 使用当开始需要与当前工作区隔离的功能工作或执行实现计划之前需要创建隔离 git worktrees 时
---
...
### 2. 检查 CLAUDE.md 或 AGENTS.md

```bash
grep -i "worktree.*director" CLAUDE.md 2>/dev/null || grep -i "worktree.*director" AGENTS.md 2>/dev/null
```
...
```bash
# Bun (优先，如果存在 bun.lock 或 bunfig.toml)
if [ -f bun.lock ] || [ -f bunfig.toml ]; then bun install; fi

# Node.js
if [ -f package.json ] && [ ! -f bun.lock ]; then npm install; fi
...
| `worktrees/` 存在 | 使用它（验证被忽略） |
| 两者都存在 | 使用 `.worktrees/` |
| 都不存在 | 检查 CLAUDE.md/AGENTS.md → 询问用户 |
| 目录未被忽略 | 添加到 .gitignore + 提交 |
| 基线测试失败 | 报告失败 + 询问 |
| 有 bun.lock | 使用 bun install |
| 有 package.json（无 bun.lock） | 使用 npm install |
| 无包管理器文件 | 跳过依赖安装 |
...
- 没有询问就继续使用失败的测试
- 当模糊时假设目录位置
- 跳过 CLAUDE.md/AGENTS.md 检查

**始终：**
- 遵循目录优先级：现有 > CLAUDE.md/AGENTS.md > 询问
- 验证目录被忽略（项目本地）
- 自动检测并运行项目设置
- 验证干净的测试基线
...
**被以下调用：**
- **executing-plans** - 执行计划前设置工作区（步骤 0）
- **subagent-driven-development** - 执行任何任务前设置工作区
- 任何需要隔离工作区的技能

**配对使用：**
- **finishing-a-development-branch** - 工作完成后清理必需

**注意：** brainstorming 和 writing-plans 不需要 worktree，它们在主仓库中创建设计文档和计划文档。worktree 只在实际执行实现时创建。