...
- 计划中包含完整代码（不是"添加验证"）
- 带预期输出的精确命令
- 使用 `superpowers:技能名` 格式引用相关技能（如 `@superpowers:test-driven-development`）
- DRY、YAGNI、TDD、频繁提交
...
**"计划完成并保存到 `docs/plans/`。两种执行方式：**

**1. 子代理驱动（此会话）** - 留在当前会话，每个任务派遣新鲜子代理，两阶段审查（规范+质量），快速迭代

**2. 批量执行（当前或新会话）** - 批量执行任务，批次间代码审查，适合较大任务
