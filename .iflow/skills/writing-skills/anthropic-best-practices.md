...
好的技能是简洁的、结构良好的，并且经过真实使用测试。本指南提供实用的编写决策建议，帮助你编写 Claude 能够有效发现和使用的技能。
...
并非技能中的每个令牌都有即时成本。启动时，只预加载所有技能的元数据（名称和描述）。Claude 只在技能变得相关时才读取 SKILL.md，并只在需要时读取其他文件。然而，在 SKILL.md 中保持简洁仍然很重要：一旦 Claude 加载它，每个令牌都与对话历史和其他上下文竞争。
...
* **两侧有悬崖的窄桥**：只有一条安全的前进道路。提供具体的护栏和精确指令（低自由度）。示例：必须按确切顺序运行的数据库迁移。
* **没有障碍物的开阔场地**：许多路径都能成功。给出大致方向并信任 Claude 找到最佳路线（高自由度）。示例：代码审查，上下文决定最佳方法。

### 使用所有你计划使用的模型进行测试

技能作为模型的补充，因此效果取决于底层模型。使用你计划使用的所有模型测试你的技能。
...
对 Opus 完美适用的可能需要更多细节才能用于 Haiku。如果你计划在多个模型上使用技能，请以适用于所有模型的指令为目标。
...
**要具体并包含关键术语**。包含技能做什么以及何时使用它的特定触发器/上下文。

每个技能恰好有一个描述字段。描述对于技能选择至关重要：Claude 使用它从可能 100+ 个可用技能中选择正确的技能。你的描述必须提供足够的细节让 Claude 知道何时选择此技能，而 SKILL.md 的其余部分提供实现细节。
...
```yaml  theme={null}
description: 从 PDF 文件提取文本和表格，填充表单，合并文档。在处理 PDF 文件或用户提到 PDF、表单或文档提取时使用。
```
...
SKILL.md 作为概述，根据需要将 Claude 指向详细材料，就像入职指南中的目录。关于渐进式披露如何工作的解释，请参阅概述中的[技能如何工作](/en/docs/agents-and-tools/agent-skills/overview#how-skills-work)。
...
对于有多个领域的技能，按领域组织内容以避免加载不相关的上下文。当用户询问销售指标时，Claude 只需要读取销售相关的模式，而不是财务或营销数据。这保持令牌使用低和上下文集中。
...
Claude 可能只部分读取从其他引用文件引用的文件。当遇到嵌套引用时，Claude 可能使用 `head -100` 等命令预览内容而不是读取整个文件，导致信息不完整。

**保持引用从 SKILL.md 深度为一级**。所有引用文件应直接从 SKILL.md 链接，以确保 Claude 在需要时读取完整文件。
...
关于此基于文件系统的架构如何启用渐进式披露的详情，请参阅下面高级部分的[运行时环境](#runtime-environment)部分。

## 工作流和反馈循环

### 为复杂任务使用工作流

将复杂操作分解为清晰的顺序步骤。对于特别复杂的工作流，提供 Claude 可以复制到其响应中并在进展时勾选的检查清单。
...
对于每个主要声明，验证它出现在源材料中。注意哪个源支持每一点。
...
此示例显示工作流如何适用于不需要代码的分析任务。检查清单模式适用于任何复杂的多步骤流程。

**示例 2：PDF 表单填充工作流**（用于有代码技能）：
...
清晰的步骤防止 Claude 跳过关键验证。检查清单帮助 Claude 和你跟踪多步工作流的进度。
...
示例帮助 Claude 比仅描述更清楚地理解期望的风格和细节程度。
...
<Tip>
  如果工作流变大或变复杂有很多步骤，考虑将它们推到单独文件中，并告诉 Claude 根据手头任务读取适当的文件。
</Tip>
...
1. **识别差距**：在没有技能的情况下用代表性任务运行 Claude。记录特定失败或缺失上下文
2. **创建评估**：构建三个测试这些差距的场景
3. **建立基线**：测量没有技能时 Claude 的表现
4. **编写最小指令**：创建刚好足够的内容来解决差距并通过评估
5. **迭代**：执行评估，与基线比较，并改进

这种方法确保你在解决实际问题而不是预期可能永远不会发生的需求。
...
<Note>
  此示例演示带简单测试评分标准的数据驱动评估。我们目前不提供内置方式来运行这些评估。用户可以创建自己的评估系统。评估是衡量技能有效性的真实来源。
</Note>

### 与 Claude 迭代开发技能

最有效的技能开发过程涉及 Claude 本身。与一个 Claude 实例（"Claude A"）一起创建将被其他实例（"Claude B"）使用的技能。Claude A 帮助你设计和改进指令，而 Claude B 在真实任务中测试它们。这有效是因为 Claude 模型既理解如何编写有效的 agent 指令，也理解 agent 需要什么信息。

**创建新技能：**

1. **没有技能完成任务**：与 Claude A 一起使用常规提示解决问题。随着工作，你会自然提供上下文、解释偏好和分享过程知识。注意你反复提供的信息。
...
3. **要求 Claude A 创建技能**："创建一个捕获我们刚才使用的 BigQuery 分析模式的技能。包括表模式、命名约定和关于过滤测试账户的规则。"
   ...
   <Tip>
     Claude 模型原生理解技能格式和结构。你不需要特殊系统提示或"编写技能"技能来让 Claude 帮助创建技能。只需要求 Claude 创建技能，它将生成结构正确的 SKILL.md 内容，带有适当的前言和主体内容。
   </Tip>

4. **审查简洁性**：检查 Claude A 是否添加了不必要的解释。要求："删除关于胜率意味着什么的解释 - Claude 已经知道那个。"

5. **改进信息架构**：要求 Claude A 更有效地组织内容。例如："组织这个以便表模式在单独的参考文件中。我们稍后可能会添加更多表。"

6. **在类似任务上测试**：在相关用例上与 Claude B（加载技能的新实例）一起使用技能。观察 Claude B 是否找到正确信息、正确应用规则并成功处理任务。

7. **基于观察迭代**：如果 Claude B 挣扎或遗漏某些东西，带具体细节返回 Claude A："当 Claude 使用此技能时，它忘记按日期过滤 Q4。我们应该添加关于日期过滤模式的部分吗？"
...
* **与 Claude B 测试**（使用技能执行真实工作的 agent）
* **观察 Claude B 的行为**并将见解带回 Claude A

1. **在真实工作流中使用技能**：给 Claude B（加载了技能）实际任务，不是测试场景

2. **观察 Claude B 的行为**：注意它在哪里挣扎、成功或做出意外选择的地方

   **示例观察**："当我要求 Claude B 提供区域销售报告时，它编写了查询但忘记过滤掉测试账户，即使技能提到了此规则。"

3. **返回 Claude A 进行改进**：分享当前 SKILL.md 并描述你观察到的。要求："我注意到当我要求区域报告时 Claude B 忘记过滤测试账户。技能提到过滤，但可能不够突出？"
...
6. **基于使用重复**：随着遇到新场景继续此观察-改进-测试循环。每次迭代基于真实 agent 行为而不是假设改进技能。
...
**为什么这种方法有效**：Claude A 理解 agent 需求，你提供领域专业知识，Claude B 通过真实使用揭示差距，迭代改进基于观察到的行为而不是假设改进技能。

### 观察 Claude 如何导航技能

随着你迭代技能，注意 Claude 在实践中实际如何使用它们。观察：

* **意外的探索路径**：Claude 是否以你没预期的方式读取文件？这可能表明你的结构不像你想象的那么直观
* **错过的连接**：Claude 是否未能遵循指向重要文件的引用？你的链接可能需要更明确或更突出
* **过度依赖某些部分**：如果 Claude 反复读取同一文件，考虑该内容是否应该在主 SKILL.md 中
* **被忽略的内容**：如果 Claude 从未访问打包文件，它可能是不必要的或在主指令中信号不好

基于这些观察而不是假设迭代。技能元数据中的"name"和"description"特别关键。Claude 在决定是否响应当前任务触发技能时使用这些。确保它们清楚描述技能做什么以及何时应该使用。
...
Unix 风格路径适用于所有平台，而 Windows 风格路径在 Unix 系统上导致错误。
...
以下部分侧重于包含可执行脚本的技能。如果你的技能只使用 markdown 指令，跳到[有效技能检查清单](#checklist-for-effective-skills)。
...
配置参数也应该被证明和文档化以避免"巫术常量"（Ousterhout 定律）。如果你不知道正确的值，Claude 如何确定它？
...
对于大多数实用脚本，执行是首选，因为它更可靠和高效。详见下面[运行时环境](#runtime-environment)部分了解脚本执行如何工作。
...
当 Claude 执行复杂、开放式任务时，它可能犯错。"计划-验证-执行"模式通过让 Claude 首先以结构化格式创建计划，然后在执行前用脚本验证该计划来早期捕获错误。

**示例**：想象要求 Claude 基于电子表格更新 PDF 中的 50 个表单字段。没有验证，Claude 可能引用不存在的字段、创建冲突值、遗漏必需字段或错误应用更新。

**解决方案**：使用上面显示的工作流模式（PDF 表单填充），但添加一个中间 `changes.json` 文件，在应用更改前验证。工作流变为：分析 → **创建计划文件** → **验证计划** → 执行 → 验证。
...
**实现提示**：使验证脚本详细，带有具体错误消息如"字段 'signature_date' 未找到。可用字段：customer_name、order_total、signature_date_signed"以帮助 Claude 修复问题。

### 包依赖

技能在代码执行环境中运行，具有平台特定限制：
...
1. **元数据预加载**：启动时，所有技能 YAML 前言中的名称和描述被加载到系统提示中
2. **按需读取文件**：Claude 使用 bash Read 工具在需要时从文件系统访问 SKILL.md 和其他文件
3. **高效执行脚本**：可以通过 bash 执行实用脚本而无需将其完整内容加载到上下文中。只有脚本的输出消耗令牌
4. **大文件无上下文惩罚**：参考文件、数据或文档在实际读取前不消耗上下文令牌

* **文件路径很重要**：Claude 像文件系统一样导航你的技能目录。使用正斜杠（`reference/guide.md`），不是反斜杠
...
* **打包全面资源**：包含完整的 API 文档、大量示例、大数据集；访问前无上下文惩罚
...
当用户询问收入时，Claude 读取 SKILL.md，看到对 `reference/finance.md` 的引用，并调用 bash 读取该文件。sales.md 和 product.md 文件保留在文件系统上，直到需要前消耗零上下文令牌。此基于文件系统的模型使渐进式披露成为可能。Claude 可以导航并选择性地加载每个任务所需的确切内容。

关于技术架构的完整详情，请参阅技能概述中的[技能如何工作](/en/docs/agents-and-tools/agent-skills/overview#how-skills-work)。
...
SKILL.md 前言只包含 `name`（最多 64 个字符）和 `description`（最多 1024 个字符）字段。完整结构详情见[技能概述](/en/docs/agents-and-tools/agent-skills/overview#skill-structure)。

### 令牌预算

保持 SKILL.md 主体在 500 行以下以获得最佳性能。如果内容超过此限制，使用前面描述的渐进式披露模式拆分为单独文件。架构详情见[技能概述](/en/docs/agents-and-tools/agent-skills/overview#how-skills-work)。
