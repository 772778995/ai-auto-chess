 ...
 *   ./render-graphs.js <skill-directory> --combine # 将所有合并为一个图表
 *
 * 从 SKILL.md 提取所有 ```dot 块并渲染为 SVG。
 * 用于帮助你的合作伙伴可视化流程。
 *
 * 需要：系统上安装 graphviz (dot)
 */
...
function extractGraphBody(dotContent) {
  // 只从 digraph 中提取主体（节点和边）
  const match = dotContent.match(/digraph\s+\w+\s*\{([\s\S]*)\}/);
  if (!match) return '';

  let body = match[1];

  // 删除 rankdir（我们将在顶层设置一次）
