 ...
 * @param timeoutMs - 最大等待时间（默认 5000ms）
 * @returns 达到计数后解析为所有匹配事件的 Promise
 *
 * 示例：
  ...
  return new Promise((resolve, reject) => {
    ...
    const check = () => {
      ...
      if (matchingEvents.length >= count) {
        ...
        reject(
          new Error(
            `等待 ${count} 个 ${eventType} 事件超时，已超过 ${timeoutMs}ms（得到 ${matchingEvents.length} 个）`
          )
        );
      } else {
        setTimeout(check, 10);
...
// 之前（不稳定）：
// ---------------
// const messagePromise = agent.sendMessage('执行工具');
